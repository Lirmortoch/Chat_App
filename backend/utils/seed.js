...
async function seed() {
  try {
    ...
    for (let i = 0; i < 300; i++) {
      ...
      const members = await postgreSql`SELECT user_id FROM chat.chats_members WHERE chat_id = ${randomChat.id}`
    ...
    const extensions = { image: 'jpg', video: 'mp4', document: 'pdf', audio: 'mp3' }
